	...
	public boolean a(World paramWorld, Random paramRandom, int paramInt1, int paramInt2, int paramInt3)
	{
		for (int i = 0; i < 20; i++) {
			...
			if ((!paramWorld.isEmpty(j, k, m)) || (
				(paramWorld.getMaterial(j - 1, k - 1, m) != Material.WATER) && (paramWorld.getMaterial(j + 1, k - 1, m) != Material.WATER) && (paramWorld.getMaterial(j, k - 1, m - 1) != Material.WATER) && (paramWorld.getMaterial(j, k - 1, m + 1) != Material.WATER)))
			{
				continue;
			}
			...
			for (int i1 = 0; i1 < n; i1++) {
				if (Block.SUGAR_CANE_BLOCK.d(paramWorld, j, k + i1, m)) {
