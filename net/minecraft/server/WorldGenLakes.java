	...
	public boolean a(World paramWorld, Random paramRandom, int paramInt1, int paramInt2, int paramInt3)
	{
			...
			for (int k = 1; k < 15; k++)
				for (int m = 1; m < 15; m++)
					for (int n = 1; n < 7; n++) {
						...
						if (d10 >= 1.0D) continue; arrayOfBoolean[((k * 16 + m) * 8 + n)] = true;
					}
		}
		int i1;
		int i2;
		...
		for (j = 0; j < 16; j++) {
			for (i1 = 0; i1 < 16; i1++) {
				for (i2 = 4; i2 < 8; i2++) {
					if ((arrayOfBoolean[((j * 16 + i1) * 8 + i2)] == 0) || 
						(paramWorld.getTypeId(paramInt1 + j, paramInt2 + i2 - 1, paramInt3 + i1) != Block.DIRT.id) || (paramWorld.b(EnumSkyBlock.SKY, paramInt1 + j, paramInt2 + i2, paramInt3 + i1) <= 0)) continue;
					BiomeBase localBiomeBase = paramWorld.getBiome(paramInt1 + j, paramInt3 + i1);
					if (localBiomeBase.A == Block.MYCEL.id)
						paramWorld.setRawTypeId(paramInt1 + j, paramInt2 + i2 - 1, paramInt3 + i1, Block.MYCEL.id);
					else paramWorld.setRawTypeId(paramInt1 + j, paramInt2 + i2 - 1, paramInt3 + i1, Block.GRASS.id);
				}
			...
			for (j = 0; j < 16; j++) {
				for (i1 = 0; i1 < 16; i1++) {
					for (i2 = 0; i2 < 8; i2++) {
						...
						if ((i4 == 0) || 
							((i2 >= 4) && (paramRandom.nextInt(2) == 0)) || (!paramWorld.getMaterial(paramInt1 + j, paramInt2 + i2, paramInt3 + i1).isBuildable())) continue;
		...
		if (Block.byId[this.a].material == Material.WATER) {
			for (j = 0; j < 16; j++) {
				for (i1 = 0; i1 < 16; i1++) {
					i2 = 4;
					if (paramWorld.u(paramInt1 + j, paramInt2 + i2, paramInt3 + i1)) {
