...
public class Block
{
	...
	public float getDamage(EntityHuman entityhuman, World world, int i, int j, int k) {
		float f = m(world, i, j, k);

		return !entityhuman.b(this) ? 1.0F / f / 100.0F : f < 0.0F ? 0.0F : entityhuman.a(this) / f / 30.0F;
	}
		...
		if (!world.isStatic) {
			...
			for (int k1 = 0; k1 < j1; k1++)
			{
				if (world.random.nextFloat() < f) {
					int l1 = getDropType(l, world.random, i1);

					if (l1 > 0)
						a(world, i, j, k, new ItemStack(l1, 1, getDropData(l)));
				}
			}
		}
	}
	...
	static
	{
		...
		Item.byId[BIG_MUSHROOM_1.id] = new ItemWithAuxData(BIG_MUSHROOM_1.id - 256, false);
		Item.byId[BIG_MUSHROOM_2.id] = new ItemWithAuxData(BIG_MUSHROOM_2.id - 256, false);
		Item.byId[MOB_SPAWNER.id] = new ItemWithAuxData(MOB_SPAWNER.id - 256, false);
