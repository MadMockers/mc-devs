...
public class ItemEnderEye extends Item
{
	...
	public boolean interactWith(ItemStack paramItemStack, EntityHuman paramEntityHuman, World paramWorld, int paramInt1, int paramInt2, int paramInt3, int paramInt4, float paramFloat1, float paramFloat2, float paramFloat3)
	{
		...
		if ((paramEntityHuman.e(paramInt1, paramInt2, paramInt3)) && (i == Block.ENDER_PORTAL_FRAME.id) && (!BlockEnderPortalFrame.e(j))) {
			if (paramWorld.isStatic) return true;
			paramWorld.setData(paramInt1, paramInt2, paramInt3, j + 4);
			paramItemStack.count -= 1;

			for (int k = 0; k < 16; k++) {
				double d1 = paramInt1 + (5.0F + d.nextFloat() * 6.0F) / 16.0F;
				double d2 = paramInt2 + 0.8125F;
				double d3 = paramInt3 + (5.0F + d.nextFloat() * 6.0F) / 16.0F;
				double d4 = 0.0D;
				double d5 = 0.0D;
				double d6 = 0.0D;
			...
			if ((i2 != 0) && (n == m + 2))
			{
				for (i4 = m; i4 <= n; i4++) {
					...
					i7 = paramWorld.getTypeId(i5, paramInt2, i6);
					i8 = paramWorld.getData(i5, paramInt2, i6);
					if ((i7 != Block.ENDER_PORTAL_FRAME.id) || (!BlockEnderPortalFrame.e(i8))) {
						i2 = 0;
						break;
					}
				}

				for (i4 = m - 1; i4 <= n + 1; i4 += 4) {
					for (i5 = 1; i5 <= 3; i5++) {
						...
						if ((i8 != Block.ENDER_PORTAL_FRAME.id) || (!BlockEnderPortalFrame.e(i9))) {
							i2 = 0;
							break;
						}
					}
