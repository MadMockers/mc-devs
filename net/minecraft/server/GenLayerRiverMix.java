	...
	public int[] a(int paramInt1, int paramInt2, int paramInt3, int paramInt4)
	{
		...
		for (int i = 0; i < paramInt3 * paramInt4; i++) {
			if (arrayOfInt1[i] == BiomeBase.OCEAN.id) {
				...
				else if ((arrayOfInt1[i] == BiomeBase.MUSHROOM_ISLAND.id) || (arrayOfInt1[i] == BiomeBase.MUSHROOM_SHORE.id))
					arrayOfInt3[i] = BiomeBase.MUSHROOM_SHORE.id;
				else arrayOfInt3[i] = arrayOfInt2[i]; 
			}
			else {
				arrayOfInt3[i] = arrayOfInt1[i];
			}
